...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    { 
        hr = AMovieSetupRegisterServer(CLSID_PushSourceDesktop, L"capture-text-overlay", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
                ...
                hr = fm->RegisterFilter(CLSID_PushSourceDesktop, L"capture-text-overlay", &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else
            {
